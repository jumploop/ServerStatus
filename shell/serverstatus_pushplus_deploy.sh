...
modify_config() {
    if [[ $# -lt 1 ]]; then
        echo -e "${red}参数错误，未能正确提供sc bot信息，请手动修改docker-compse.yml中的bot信息 ${plain}"
        exit 1
    fi
    ...
    *)
    ...
    sed -i "s/\${PP_BOT_TOKEN}/${pp_bot_token}/" docker-compose.yml
    sed -i "s/\${SERVER_DOMAIN}/${sss_adder}/" docker-compose.yml
    echo -e "> 修改config.json"
    sed -i "s#yourSMSurl#www.pushplus.plus/send?token=${pp_bot_token}\&title=ServerStatus\&content=#g" server/config.json
}
...
install_dashboard() {
    ...
    [ ! -d web ] && mkdir web
    wget --no-check-certificate -O docker-compose.yml ${GITHUB_RAW_URL}/docker-compose/docker-compose-pushplus.yml >/dev/null 2>&1
    wget --no-check-certificate -O Dockerfile ${GITHUB_RAW_URL}/Dockerfile >/dev/null 2>&1
    wget --no-check-certificate -O Dockerfile-pushplus ${GITHUB_RAW_URL}/docker-compose/Dockerfile-pushplus >/dev/null 2>&1
    wget --no-check-certificate -O bot-pushplus.py ${GITHUB_RAW_URL}/plugin/bot-pushplus.py >/dev/null 2>&1
    wget --no-check-certificate -O config_manager.py ${GITHUB_RAW_URL}/plugin/config_manager.py >/dev/null 2>&1
...
nodes_mgr() {
    python3 config_manager.py -c server/config.json
}
