...
modify_config() {
    ...
    *)
    ...
    sed -i "s/\${SC_BOT_TOKEN}/${sc_bot_token}/" docker-compose.yml
    sed -i "s/\${SERVER_DOMAIN}/${sss_adder}/" docker-compose.yml
    echo -e "> 修改config.json"
    sed -i "s#yourSMSurl#sctapi.ftqq.com/${sc_bot_token}.send?title=ServerStatus\&desp=#g" server/config.json
}
...
install_dashboard() {
    ...
    [ ! -d web ] && mkdir web
    wget --no-check-certificate -O docker-compose.yml ${GITHUB_RAW_URL}/docker-compose/docker-compose-serverchan.yml >/dev/null 2>&1
    wget --no-check-certificate -O Dockerfile ${GITHUB_RAW_URL}/Dockerfile >/dev/null 2>&1
    wget --no-check-certificate -O Dockerfile-serverchan ${GITHUB_RAW_URL}/docker-compose/Dockerfile-serverchan >/dev/null 2>&1
    wget --no-check-certificate -O bot-serverchan.py ${GITHUB_RAW_URL}/plugin/bot-serverchan.py >/dev/null 2>&1
